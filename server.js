...
app.get('/highest', (req, res) => {
    res.json({num : latestNum})
})


app.post('/num', (req, res) => {
    currentNum = req.body.num;
    const numComic = fetch(`http://xkcd.com/${currentNum}/info.0.json`);
    numComic.then(response => {
...
app.get('/next', (req, res) => {
    currentNum++;
    const nextComic = fetch(`http://xkcd.com/${currentNum}/info.0.json`);
    nextComic.then(response => {
        return response.json();
    }).then(data => {
        res.json(data);
    }).catch(error => {
